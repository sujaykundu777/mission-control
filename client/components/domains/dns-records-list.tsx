...
export function DNSRecordsList({ records, domainId }: DNSRecordsListProps) {
  ...
  if (records.length === 0) {
    return (
      <Card className="border-border bg-card p-12 text-center">
        <p className="mb-4 text-muted-foreground">No DNS records added yet</p>
  ...
  return (
    <div className="space-y-4">
      <div className="mb-4 hidden gap-4 rounded-lg bg-secondary p-4 md:grid md:grid-cols-5">
        <div>
      ...
      {records.map((record) => (
        <Card key={record.id} className="border-border bg-card p-4">
          <div className="grid grid-cols-1 items-center gap-4 md:grid-cols-5">
            ...
            <div>
              ...
              <p className="break-all font-mono text-sm text-foreground">{record.name || "@"}</p>
            </div>
