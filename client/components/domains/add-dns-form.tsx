...
export function AddDNSForm({ domainId, recordId, isEdit = false }: AddDNSFormProps) {
  ...
  const existingRecord = isEdit && recordId ? domain?.dnsRecords.find((r) => r.id === recordId) : null
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="p-6 bg-card border-border">
          <h2 className="text-lg font-semibold text-foreground mb-4">DNS Record Details</h2>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Record Type *</label>
                <select
                  name="type"
                  value={formData.type}
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Record Name</label>
                <Input
                  type="text"
                  name="name"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">TTL (Time to Live)</label>
                <Input
                  type="number"
                  name="ttl"
                  placeholder="3600"
        ...
        <div className="bg-secondary/50 border border-border rounded-lg p-4">
          <p className="text-xs text-muted-foreground">
            <span className="font-semibold">Tip:</span> Make sure to update these records with your domain registrar
            for them to take effect. Keep the TTL value between 300-3600 seconds for most use cases.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {isSubmitting ? (isEdit ? 'Updating...' : 'Adding...') : isEdit ? 'Update Record' : 'Add Record'}
          </Button>
          <Link href={`/domains/${domainId}`}>
