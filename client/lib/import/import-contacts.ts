  ...
  for (let i = 1; i < lines.length; i++) {
    try {
      ...
      const relationshipType =
        relationshipTypeIndex !== -1 ? values[relationshipTypeIndex] : undefined;
      const martialStatus = (martialStatusIndex !== -1 ? values[martialStatusIndex] : "Single") as
        | "Single"
        | "Married"
        | "Divorce"
        | "Widowed";
      ...
      const status = (statusIndex !== -1 ? values[statusIndex] : "active") as
        | "active"
        | "inactive"
        | "archived";
...
export async function importContactsFromJSON(jsonContent: string): Promise<ImportCSVResult> {
  ...
  try {
    ...
    const existingEmails = new Set(
      await (await storage.getContacts()).map((c) => c.email.toLowerCase()),
    );
